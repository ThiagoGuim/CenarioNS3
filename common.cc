...
const TrafficTypeQueueMap_t&
TrafficType2QueueMap (std::vector<uint8_t> sliceIds, uint8_t scenarioConfig)
{
  static TrafficTypeQueueMap_t queueByTrafficType;

  // Populating the map at the first time.
  switch(scenarioConfig){

    case 1:
      if (queueByTrafficType.empty ())
        { 

          uint8_t numQueue = 0;
          for (uint8_t sliceId = 1; sliceId <= sliceIds.size(); sliceId++){

            for (uint8_t t = 0; t < N_TRAFFIC_TYPES; t++){
              
              key_t sliceIdTrafficId;
              sliceIdTrafficId = std::make_tuple(sliceId, t);

              queueByTrafficType.insert (std::make_pair (sliceIdTrafficId,   numQueue));
...
Ipv4Header::DscpType
trafficTypeToDscpHeader (TQosType ttype)
{ 
  switch (ttype)
    {
    case TQosType::AF:
    ...
    default:
      NS_ABORT_MSG ("No QoS type mapped value for DSCP " << ttype);
    }
}
