...
#define N_TRAFFIC_TYPES (static_cast<int> (TQosType::ALL))
#define N_TRAFFIC_TYPES_ALL (static_cast<int> (TQosType::ALL) + 1)
...
std::string TQosTypeStr (TQosType type);
// ----------------------------------------------------------------------------
...
/** Map saving trafficType value / OpenFlow queue id. */
typedef std::tuple <uint8_t, uint8_t> key_t;
typedef std::map<key_t, uint8_t> TrafficTypeQueueMap_t;
...
const TrafficTypeQueueMap_t& TrafficType2QueueMap (std::vector<uint8_t> sliceIds, uint8_t scenarioConfig);
Ipv4Header::DscpType trafficTypeToDscpHeader (TQosType ttype);
