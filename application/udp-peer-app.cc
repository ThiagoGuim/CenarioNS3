...
UdpPeerApp::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UdpPeerApp")
    ...
    .AddAttribute ("TQosType", "Different types of traffic.",
                   ...
                   MakeEnumAccessor (&UdpPeerApp::m_tQosType),
...
uint8_t 
UdpPeerApp::GetSliceId(void){
  return m_sliceId;
}

TQosType
UdpPeerApp::GetTQosType(void){
...
UdpPeerApp::SendPacket ()
{
  ...
  SliceTag tag (m_sliceId, m_qosType, m_tQosType);
  // std::cout << "Slice id do pacote " << m_sliceId << std::endl;
